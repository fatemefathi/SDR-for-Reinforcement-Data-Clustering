%function [W, y, S, evs] = PropsedMethod2(X, c, d, k, r)
 clc;
 close all;
 clear all;

%Determine input and label and number of class
...
[R N]=size(X);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%compute initial Sij and Ls and P by computeprob
...
%%%% set landa and beta and beat2 based on dataset
...
for iter = 1:Iter
    ...
    Q=0.5*diag(1./sqrt(diag(W*W')));
    M1 = (X*L*X');
    M= M1+beta2*Q;
    W = eig1(M, d, 0, 0);
    W = W*diag(1./sqrt(diag(W'*W)))+Q*W;
